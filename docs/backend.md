...
The backend is a thin host for the daily-bar backtest engine — a serving layer plus a scheduler that runs the engine's modules in order. It does not do live scanning, does not run an LLM/SLM, and does not send alerts; those are v2+ (see Future work below).
...
backend/
  app/
    ...
    breakout/
      breakout_engine.py     — state machine (see engine.md)
      scoring.py               — breakout score + hard rejection rules (see engine.md)
...
|---|---|---|
| `/breakout-candidates` | GET | Symbols currently in a non-terminal breakout state |
...
- Live intraday scanner loop (09:15–15:30, per-minute candle/indicator updates, 5-minute breakout scans) once a live broker/WebSocket feed exists.
